...
class TizenDriver extends BaseDriver {
  ...
  async createSession(caps) {
    try {
      ...
      this.sdb = await helpers.createSDB(
        this.opts.udid,
        this.opts.emPort,
        this.opts.sdbPort,
  ...
  async checkPackagePresent() {
    ...
    if (
      !(await this.sdb.shell([
        `app_launcher --list | grep ${this.opts.appPackage}`,
      ]))
    ) {
      log.errorAndThrow(
        `Could not find package ${this.opts.appPackage} on the device`
      );
    }
  }
