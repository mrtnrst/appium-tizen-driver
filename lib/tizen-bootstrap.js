...
class TizenBootstrap {
  ...
  async connectSocket() {
    try {
      ...
      log.errorAndThrow(
        `Error occured while reconnection TizenBootstrap. Original error: ${err}`
      );
    }
  }
  ...
  async sendCommand(type, extra = {}) {
    ...
    return await new B((resolve, reject) => {
      ...
      log.debug(
        `Sending command to tizen: ${_.truncate(cmdJson, {
          length: 1000,
        }).trim()}`
      );
  ...
  async installUIAutomator() {
    ...
    let tpkPath = path.resolve(
      rootDir,
      'uiautomator',
      `${this.uiautomator}-${this.uiautomatorVersion}`
    );
